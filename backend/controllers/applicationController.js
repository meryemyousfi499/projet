...
exports.apply = async (req, res) => {
  try {
    const subject = await Subject.findById(req.params.subjectId).exec(); // L11 ✅
    ...
    const group = await Group.findOne({ membres: req.user.id })  // L17 ✅
    ...
    const existing = await Application.findOne({ groupId: group._id, sujetId: req.params.subjectId }).exec(); // L23 ✅
...
exports.getApplications = async (req, res) => {
  try {
    ...
    if (req.user.role === 'ROLE_STUDENT') {
      const group = await Group.findOne({ membres: req.user.id }).exec(); // L52 ✅
      if (!group) return res.json({ success: true, data: [] });
      query.groupId = group._id;
    }

    if (req.user.role === 'ROLE_SUPERVISOR') {
      const subjects = await Subject.find({ encadrantId: req.user.id }).select('_id').exec(); // L58 ✅
...
exports.updateApplication = async (req, res) => {
  try {
    ...
    if (statut === 'accepté') {
      ...
      await Subject.findByIdAndUpdate(app.sujetId._id, { statut: 'complet' }).exec(); // L111 ✅
