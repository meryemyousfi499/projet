...
exports.register = async (req, res) => {
  try {
    ...
    const existingUser = await User.findOne({ email }).exec(); // L25 ✅
...
exports.login = async (req, res) => {
  try {
    ...
    const user = await User.findOne({ email }).select('+motDePasse').exec(); // L38 ✅
...
exports.getMe = async (req, res) => {
  const user = await User.findById(req.user.id).exec(); // L50 ✅
...
exports.changePassword = async (req, res) => {
  try {
    ...
    const user = await User.findById(req.user.id).select('+motDePasse').exec(); // L67 ✅
...
exports.forgotPassword = async (req, res) => {
  try {
    const user = await User.findOne({ email: req.body.email }).exec(); // L80 ✅
