...
exports.getAllSubjects = async (req, res) => {
  try {
    ...
    const total = await Subject.countDocuments(query).exec(); // L15 ✅
...
exports.createSubject = async (req, res) => {
  try {
    ...
    const admins = await User.find({ role: 'ROLE_ADMIN' }).exec(); // L41 ✅
...
exports.updateSubject = async (req, res) => {
  try {
    ...
    subject = await Subject.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true }).exec(); // L58 ✅
    res.json({ success: true, data: subject });
  } catch (err) {
    res.status(400).json({ success: false, message: err.message });
  }
...
exports.getMySupervisorSubjects = async (req, res) => {
  try {
    const subjects = await Subject.find({ encadrantId: req.user.id })
      .sort({ createdAt: -1 })
      .exec(); // L96 ✅
