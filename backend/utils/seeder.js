...
const seed = async () => {
  ...
  await Subject.deleteMany({}).exec();
  await Application.deleteMany({}).exec();
  await Project.deleteMany({}).exec();
  await Milestone.deleteMany({}).exec();
  await Message.deleteMany({}).exec();
  await Notification.deleteMany({}).exec();
  await Group.deleteMany({}).exec();

  await User.create({ nom:'Admin', prenom:'Super', email:'admin@pfe.com', motDePasse:'Admin@123', role:'ROLE_ADMIN', departement:'Administration', statut:'actif' });
  ...
  const [subj1, , subj3] = await Subject.create([
  ...
  await Subject.findByIdAndUpdate(subj1._id, { statut: 'complet' }).exec();
  ...
  for (let i = 0; i < milestones.length; i++) {
    await Milestone.create({
      ...
      nomEtape: milestones[i],
      ordre: i + 1,
      statut: getMilestoneStatut(i) // ✅ fonction utilisée ici
    });
  }
