...
// ✅ L383 — Cognitive Complexity réduite : extraction des sous-rendus du chat
const renderChatMessage = (msg, idx, chatMessages, userId, formatMsgTime) => {
    ...
    <div key={msg._id} style={{ display: 'flex', flexDirection: isMe ? 'row-reverse' : 'row', gap: 10, alignItems: 'flex-end' }}>
      ...
      <div style={{ maxWidth: '65%' }}>
        ...
        <div style={{
          ...
          {/* ✅ L504 — Ternaire imbriqué extrait */}
...
// ✅ L504 — Ternaire imbriqué extrait en fonction indépendante
...
// ✅ L267 — Ternaire imbriqué extrait en fonction indépendante
const renderChatList = (chatMessages, userId, formatMsgTime, chatBottomRef) => {
  ...
  return chatMessages.map((msg, idx) =>
    renderChatMessage(msg, idx, chatMessages, userId, formatMsgTime)
  );
};
...
export default function ProjectDetailPage() {
  ...
  // ✅ L302 — Ternaire imbriqué extrait
  const getTimelineDotClass = (statut) => {
    if (statut === 'terminé') return 'done';
    if (statut === 'en cours') return 'active';
    return 'pending';
  };

  // ✅ L474 — Ternaire imbriqué extrait
  const getGradeLabel = (note) => {
    if (note >= 16) return '🏆 Excellent';
    if (note >= 12) return '✅ Bien';
    if (note >= 10) return '⚠️ Passable';
    return '❌ Insuffisant';
  };

  // ✅ L15 — Cognitive Complexity réduite : extraction du rendu des onglets
  const renderTabContent = () => {
    if (activeTab === 'milestones') return renderMilestones();
    if (activeTab === 'deliverables') return renderDeliverables();
    if (activeTab === 'messages') return renderMessages();
    if (activeTab === 'evaluation') return renderEvaluation();
    return null;
  };
      ...
      <div className="timeline">
        {milestones.map(m => (
          <div key={m._id} className="timeline-item">
            <div className={`timeline-dot ${getTimelineDotClass(m.statut)}`}></div>
    ...
    <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
      <div style={{ display: 'flex', flexDirection: 'column', height: 520 }}>
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 8 }}>
          {/* ✅ L267 — Ternaire imbriqué extrait dans renderChatList */}
          {renderChatList(chatMessages, userId, formatMsgTime, chatBottomRef)}
    ...
    <div className="card">
      ...
      {!evaluation ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          {/* ✅ L491 — Array index remplacé par clé stable */}
          {[
            ...
            <div key={item.label} style={{ padding: 20, background: '#f8fafc', borderRadius: 12, border: '1px solid var(--border)' }}>
          ...
          {evaluation.noteFinale != null && (
            <div style={{ gridColumn: '1/-1', padding: 24, background: 'linear-gradient(135deg, #4f46e5, #0ea5e9)', borderRadius: 12, textAlign: 'center', color: 'white' }}>
              ...
              <div style={{ fontSize: 13, opacity: 0.8, marginTop: 4 }}>
                {getGradeLabel(evaluation.noteFinale)}
              </div>
            </div>
          )}
        </div>
  ...
  return (
    <div>
      ...
      {/* ✅ L15 — Cognitive Complexity réduite : un seul appel */}
      {renderTabContent()}
