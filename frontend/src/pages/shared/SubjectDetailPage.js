...
export default function SubjectDetailPage() {
  ...
  // ✅ L73 — Ternaire imbriqué extrait
  const getStatusBadge = (statut) => {
    if (statut === 'validé') return 'badge-success';
    if (statut === 'complet') return 'badge-gray';
    return 'badge-warning';
  };

  // ✅ L164 — Ternaires imbriqués extraits
  const renderApplySection = () => {
    ...
    if (myGroup.chef?._id !== user._id && myGroup.chef !== user._id) {
      return (
        <div>
          <div style={{ fontSize: 40, marginBottom: 8 }}>🔒</div>
        ...
        <div>
          <div className="card" style={{ marginBottom: 20 }}>
            <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
              {/* ✅ L73 — Ternaire imbriqué remplacé par fonction */}
              <span className={`badge ${getStatusBadge(subject.statut)}`}>
                {subject.statut}
              </span>
            </div>
        ...
        <div>
          ...
          {user.role === 'ROLE_STUDENT' && subject.statut === 'validé' && (
            <div className="card" style={{ textAlign: 'center' }}>
              {/* ✅ L164 — Ternaires imbriqués remplacés par fonction */}
              {renderApplySection()}
            </div>
          )}
        </div>
