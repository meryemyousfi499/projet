...
export default function SubjectsPage() {
  ...
  // ✅ L162 — Ternaire imbriqué extrait
  const renderSubjectList = () => {
    if (loading) {
      return <div className="loading-spinner"><div className="spinner"></div><span>Chargement...</span></div>;
    }
        ...
        {subjects.map(subject => (
          <div key={subject._id} className="card" style={{ display: 'flex', flexDirection: 'column', gap: 16, transition: 'transform 0.2s, box-shadow 0.2s' }}
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              {/* ✅ L156 — Ternaire imbriqué extrait */}
              {user.role === 'ROLE_STUDENT' && renderApplyButton(subject)}
            </div>
          </div>
        ))}
      </div>
  ...
  // ✅ L156 — Ternaire imbriqué extrait
  const renderApplyButton = (subject) => {
    if (hasApplied(subject._id)) {
      return (
  ...
  return (
    <div>
      ...
      {/* ✅ L162 — Ternaire imbriqué remplacé par fonction */}
      {renderSubjectList()}
